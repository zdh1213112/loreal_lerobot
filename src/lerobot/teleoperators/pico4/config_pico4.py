...
class Pico4Config(TeleoperatorConfig):
    ...
    Attributes:
        ...
        target_tcp_drift_max_deg: Max allowed angle between Python's last commanded _target_quat
                                  ...
                                  Note: steady-state envelope enforcement lives on the robot side
                                  (BiFlexivRizon4RT.send_action) so caller dicts and recorded actions
                                  reflect the safe envelope without polluting teleop's internal
                                  _target_quat tracking.
    """
