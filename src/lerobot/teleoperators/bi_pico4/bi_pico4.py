...
class BiPico4(Teleoperator):
    ...
    def connect(
        ...
        try:
            ...
            if self._xrt is not None and not self._is_connected:
                try:
                    self._xrt.close()
                except Exception:
                    pass
    ...
    def get_action(
        ...
        action = {f"left_{k}": v for k, v in left_action.items()} | {
            f"right_{k}": v for k, v in right_action.items()
        }
        action["left_arm.enabled"] = bool(self._left_pico4._enabled)
        action["right_arm.enabled"] = bool(self._right_pico4._enabled)
        return action
