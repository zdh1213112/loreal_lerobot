...
class RecordConfig:
    ...
    def __post_init__(self):
        ...
        # XenseFlare and BiXenseFlareGrippers act as robot-only data collection devices,
        # so they don't need a separate teleoperator or policy.
        if self.teleop is None and self.policy is None:
            if self.robot.type not in {"xense_flare", "bi_xense_flare_grippers"}:
                raise ValueError(
...
@safe_stop_image_writer
def bi_xense_flare_grippers_record_loop(
    robot: Robot,
    events: dict,
    fps: int,
    dataset: LeRobotDataset | None = None,
    ...
    """Record loop for the bi_xense_flare_grippers robot-only data collector."""
    if dataset is not None and dataset.fps != fps:
        raise ValueError(
            f"The dataset fps should be equal to requested fps ({dataset.fps} != {fps})."
        )
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    if (
        ...
        and cfg.robot.type in {"bi_flexiv_rizon4_rt", "bi_dobot_nova5_dh"}
        and teleop is not None
    ):
        if cfg.robot.type == "bi_dobot_nova5_dh":
            ...
            if robot.config.control_mode != BiDobotNova5DHControlMode.CARTESIAN_MOTION:
                raise ValueError(
                    f"BiPico4 recording requires CARTESIAN_MOTION mode, "
    ...
    if teleop is not None:
        if (
            ...
            or (
                teleop_type == "btgamepad" and cfg.robot.type == "pylibfranka_research3"
            )
        ):
            teleop.connect(current_tcp_pose_quat=robot.get_current_tcp_pose_quat())
            logger.info("Teleop initialized with robot EEF pose.")
    ...
    try:
        with VideoEncodingManager(dataset):
            ...
            while (
                ...
                # Use specialized record loop for XenseFlare-style robot-only data collection devices.
                if cfg.robot.type == "xense_flare":
                    ...
                    bi_xense_flare_grippers_record_loop(
                        robot=robot,
                        events=events,
                        fps=cfg.dataset.fps,
                        dataset=dataset,
