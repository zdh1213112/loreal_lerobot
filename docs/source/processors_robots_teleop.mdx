...
Each processor step can declare what PolicyFeatures it modifies in the `transform_features` method, so we know what features we can expect when we run the processor. These features can then be aggregated and used to create the dataset features.

Below is an example of how we aggregate and merge features:
